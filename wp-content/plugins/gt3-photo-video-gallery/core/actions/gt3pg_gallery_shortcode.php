<?php
	...
	function gt3pg_gallery_shortcode( $attr ) {
		...
		$atts  = shortcode_atts( apply_filters( 'gt3pg_allowed_shortcode_atts', array(
			...
			'ids'            => '',
			'gt3_gallery'    => '',
			'link'           => $gt3_photo_gallery['gt3pg_link_to'],
			'size'           => $gt3_photo_gallery['gt3pg_size'],
			'columns'        => $gt3_photo_gallery['gt3pg_columns'],
			'real_columns'   => $gt3_photo_gallery['gt3pg_columns'],
			'orderby'        => 'post__in',
			'rand_order'     => $gt3_photo_gallery['gt3pg_rand_order'],
			'is_margin'      => 'default',
			'margin'         => $gt3_photo_gallery['gt3pg_margin'],
			'thumb_type'     => $gt3_photo_gallery['gt3pg_thumbnail_type'],
			'corners_type'   => $gt3_photo_gallery['gt3pg_corner_type'],
			'border_type'    => $gt3_photo_gallery['gt3pg_border'],
			'border_size'    => $gt3_photo_gallery['gt3pg_border_size'],
			'border_padding' => $gt3_photo_gallery['gt3pg_border_padding'],
			'border_col'     => $gt3_photo_gallery['gt3pg_border_col'],
		) ), $attr, 'gallery' );
		$atts  = apply_filters( 'gt3pg_atts', $atts );


		switch ( $atts['border_type'] ) {
			case 1:
			case 'on':
				$atts['border_type'] = 'on';
				break;
		...
		$atts['thumb_type'] = apply_filters( 'tg3pg_before_render_thumb_type', $GLOBALS["gt3_photo_gallery_defaults"]['gt3pg_thumbnail_type'], $atts );

		$atts['link'] = apply_filters( 'tg3pg_before_render_link', $GLOBALS["gt3_photo_gallery_defaults"]['gt3pg_link_to'], $atts );
		...
		foreach ( $attachments as $id => $attachment ) {
			...
			if ( $atts['gt3_gallery'] != '' ) {
				...
				$gallery_json_item->description = ( trim( $attachment->post_content ) );
				...
				if ( $item == null ) {
					...
					$image_output =
						'<div class="gt3pg_img_wrap ' . $orientation . '" style="background-image: url(' . $media_url . ');" data-width="' . $img_src_orig[1] . '" data-height="' . $img_src_orig[2] . '" data-i="' . $i . '">
