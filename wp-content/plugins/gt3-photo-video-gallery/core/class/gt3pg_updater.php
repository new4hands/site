<?php
	...
	if ( ! class_exists( 'gt3pg_updater' ) ) {
		...
		class gt3pg_updater {
			...
			protected $store_url = 'https://gt3wpgallery.com/';
			protected $license = '';
			...
			protected function actions() {
				if ( class_exists( 'GT3_EDD_SL_Plugin_Updater' ) ) {
					...
					add_action( 'admin_notices', function () {
						$msg   = 'Update unaviable for plugin ' . $this->plugin_name;
						$class = 'notice notice-warning gt3pg_error_notice';
						echo '<div class="' . $class . '"><p>' . $msg . '</p><pre></pre></div>';
					} );
				}
				$this->inline_actions();
			...
			public function plugin_row_license_missing( $plugin_data, $version_info ) {
				...
				if ( 'valid' != $this->status ) {
					...
					echo '&nbsp;<strong><a href="' . esc_url( $this->get_menu_page() ) . '">' . __( 'Enter valid license key for automatic updates.', 'gt3pg' ) . '</a></strong>';
				}
			...
			public function admin_notices() {
				if ( isset( $_GET['sl_activation'] ) && ! empty( $_GET['message'] ) ) {
					switch ( $_GET['sl_activation'] ) {
						case 'false':
							echo '<div class="error"><p>' . urldecode( $_GET['message'] ) . '</p></div>';
							break;
						case 'true':
						default:
...
//						if ($response instanceof WP_Error) $error_message = $response->get_error_message();
//						else $error_message = __( 'An error occurred, please try again.' );
						$message = ( is_wp_error( $response ) && ! empty( $error_message ) ) ? $error_message : __( 'An error occurred, please try again. ' , 'gt3pg' );
					} else {
						...
						if ( false === $license_data->success ) {
							switch ( $license_data->error ) {
								case 'expired' :
									$message = sprintf(
										__( 'Your license key expired on %s.' , 'gt3pg'),
										date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
									);
									break;
								case 'revoked' :
									$message = __( 'Your license key has been disabled.' , 'gt3pg');
									break;
								case 'missing' :
									$message = __( 'Invalid license.', 'gt3pg' );
									break;
								case 'invalid' :
								case 'site_inactive' :
									$message = __( 'Your license is not active for this URL.', 'gt3pg' );
									break;
								case 'item_name_mismatch' :
									$message = sprintf( __( 'This appears to be an invalid license key for %s.', 'gt3pg' ), $this->plugin_name );
									break;
								case 'no_activations_left':
									$message = __( 'Your license key has reached its activation limit.', 'gt3pg' );
									break;
								case 'invalid_item_id':
									$message = __( 'Product not found.', 'gt3pg' );
									break;
								default :
									$message = __( 'An error occurred, please try again.' , 'gt3pg');
									break;
							}
						}
					}
