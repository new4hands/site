...
class gt3pg_admin_mix_tab_control extends gt3classStd {
	...
	public function render() {
		?>
        <div class="gt3pg_stand_setting gt3pg_admin_mix-tab-control <?=$this->main_wrap_class?>">
            <div class="gt3pg_innerpadding">
                <label class="gt3pg_setting">
                    <h2 class="gt3pg_option_heading"><?=apply_filters( 'render_admin_mix_tab_control_' . $this->name . '_title', $this->title )?></h2>
                    <p><?=apply_filters( 'gt3_render_admin_mix_tab_control_' . $this->name . '_description', $this->description )?></p>
					<?php do_action( 'gt3_render_admin_mix_tab_control_' . $this->name . '_before_option' ); ?>
                    <div class="gt3pg_admin_input <?=$this->input_wrap_class?>">
						<?=$this->option;?>
                    </div>
