...
function insta_gallery($atts)
{
    ...
    $results .= '<div class="ig-block" id="ig-block-' . $gid . '" data-insgalid="' . $gid . '" data-source="' . $insta_source . '">';
...
function load_ig_item()
{
    if (current_user_can('administrator')) {
        // loading session for AJAX request - admin only
        if (! session_id()) {
            session_start();
        }
    }
    
    if (! isset($_POST['insgalid'])) {
        return;
    }
    $gid = (int) $_POST['insgalid'];
    $InstaGalleryItems = get_option('insta_gallery_items');
    if (! isset($InstaGalleryItems[$gid])) {
        return;
    }
    ...
    if ($IGItem['ig_select_from'] == 'username') { // get from username
        if (! empty($IGItem['insta_user'])) { // valid Instagram Username
            $sk = 'user_' . $IGItem['insta_user']; // session key
        ...
        if (! empty($IGItem['insta_tag'])) { // valid Instagram Tag;
            $sk = 'tag_' . $IGItem['insta_tag']; // session key
